...
class FP_profile:
    ...
    def __str__(self):
        ...
        keys=self.convolver_funcs.keys()
    ...
    def axial_helper(self, outerbound, innerbound, epsvals, destination, peakpos=0, y0=0, k=0):
        ...
        if abs(outerbound-innerbound)<2*dx: #peak has been squeezed out, nothing to do
        ...
        if min(intg[:-1]) < -1e-10*max(intg[:-1]): #intensities are never less than zero!
            print("bad parameters:", (5*"%10.4f") %(peakpos, innerbound, outerbound, k, y0))
    ...
    def conv_axial(self):
        ...
        if kwargs["axDiv"]!="full": #no axial divergence, transform of delta fn
            axfn[:]=1
            return axfn
        else:
            xx=type("data",(), kwargs)
    ...
    def conv_emission(self):
        ...
        gfwhm2s=(
            ...
            (xx.strain_gauss * math.tan(theta))**2 +
    ...
    def conv_size_distribution(self):
        ''' 
        create convolution kernel to account for size distribution
        for now, lets just assume its spherical particles with log-normal
        size distribution
        once this works, then other distributions and particle shapes will
        be incorporated.
        @author Saransh Singh
        @date 08/05/2020
        @ details: see "Diffraction line profiles from polydisperse crystalline 
        systems, Acta Cryst. (2001). A57, 604--613"
        '''
        from scipy.special import erfc

        me=self.get_function_name()
        flag, buf=self.get_conv(me, self.output_gaussian_smoother_bins_sigma,
            format=numpy.float)
        if flag: return buf #already computed
        mu  = 1.0#self.lognormal_mean
        sig = 0.1#self.lognormal_std
        M = lambda n : numpy.exp(n*mu + sig*(n**2/2))
        H = numpy.array([1., -1.5, 0., 0.5])
        A = numpy.zeros(self.twothetasamples.shape[0])
        L = numpy.linspace(0.01,1000,self.twothetasamples.shape[0])

        for i in range(4):
            A += H[i] * erfc((numpy.log(L)-mu-(3-i)*sig**2)/
                 numpy.sqrt(2)/sig) * M(3-i) * L**i / 2 / M(3) 

        conv=best_rfft(A)
        conv[1::2] *= -1 #flip center
        conv /= numpy.amax(numpy.abs(conv))
        return conv
    ...
    def compute_line_profile(self, convolver_names=None, compute_derivative=False):
        ...
        if(d2p*alpha > 20):
            eps_corr1 = 1.0/self.window_fullwidth
        elif(d2p*alpha < -20):
            eps_corr1 = -1.0/self.window_fullwidth
        else:
    ...
    #
    #  This rebuilds the class instance so it is ready to use on unumpyickling.
...
if __name__=='__main__' and sys.argv.pop(-1)=='plot':
    ...
    wt = 1e4 * l[:,1]/numpy.trapz(l[:,1])

    la = 1e-10 * 12.39841984/la
    mat_wavelengths = la
    mat_lor_widths = numpy.array([10e-20]*la.shape[0])
    mat_gauss_widths = numpy.array([10.0e-20]*la.shape[0])
    mat_intensities = wt #numpy.array((1.0,0.52))
    ...
    p.set_parameters(convolver="emission",
        ...
        emiss_gauss_widths=mat_gauss_widths,
        emiss_lor_widths=mat_lor_widths,
        crystallite_size_gauss=80e-9,
        crystallite_size_lor=80e-9
    )
    # p.set_parameters(convolver="axial",
    ...
    profiles=[]
    for ww in (20,):
        for twotheta_x in [9.66918231, 11.16944371, 15.821195  , 18.57434078, 19.40807977,
         22.44680771, 24.49072874, 25.13722409, 27.58168679, 29.29106634,
         29.29106634, 31.95448163, 33.46084913, 33.94977652, 33.94977652,
         35.84677273, 37.21420703, 37.66052876, 39.40273891, 40.66810866,
         40.66810866, 41.08270122]:

            #put the compute window in the right place and clear all histories
            p.set_window(twotheta_output_points=1000,
            ...
            p.set_parameters(twotheta0_deg=twotheta_x,
                dominant_wavelength=1.56404e-10,
                diffractometer_radius=2170.5e-3)
        
            for anglescale in (0.05,):
                p.set_parameters(equatorial_divergence_deg=0.5*anglescale)
                p.set_parameters(convolver="axial",
                    angI_deg=anglescale, angD_deg=anglescale,
                )
                profiles.append((twotheta_x, 0.5*anglescale, 2.5*anglescale, p.compute_line_profile()))

            # print >> sys.stderr, p
    print( sys.stderr, "execution time=", time.time()-t_start)

    # from graceplot import GracePlot
    # from graceplot.GracePlot import Data, Tick, TickLabel, Symbol, symbols, Line, lines, colors
    from matplotlib import pyplot as plt

    # gp=GracePlot.GracePlot()
    datasets=[]
    for idx, (twotheta, eqdiv, soller, result) in enumerate(profiles):
        ...
        # ds1=Data(x=result.twotheta_deg, y=peaknorm, legend="%.1f : %.1f" % (eqdiv, soller) )
        ds1 = numpy.array([result.twotheta_deg, peaknorm])
        datasets.append(ds1.T)
    # gp, axs = plt.subplots(idx)
    # for i in range(idx):
    #     axs[i].plot(datasets[i])
        # gp[0].legend()
